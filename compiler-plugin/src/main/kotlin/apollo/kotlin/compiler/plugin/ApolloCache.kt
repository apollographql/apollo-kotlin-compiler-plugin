...
class ApolloCache(userScalars: Map<String, ScalarInfo>) {
  ...
  fun getFragment(name: String) = fragments.get(name) ?: error("Apollo: unresolved fragment '$name'")
  fun getFragmentDefinition(name: String) = fragmentDefinitions.get(name) ?: error("Apollo: unresolved fragment definition '$name'")
