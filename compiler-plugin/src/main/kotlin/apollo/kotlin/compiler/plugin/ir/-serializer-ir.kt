...
fun IrBuilder.generateReadProperty(
  ...
  return if (property.isFragment ) {
     ...
     * }
     */
    if (property.type is ApNullableType) {
      val containsKey =
      ...
      apIrCall(fromJson, adapter, irGet(elementParameter))
    }
  } else {
...
fun IrBuilder.generateWriteProperty(
  ...
  return if (property.isFragment) {
    ...
    if (property.type is ApNullableType) {
      ...
      apIrCall(CallableIds.mergeFragment, irGet(mapVariable), jsonElement)
    }
  } else {
...
fun IrBuilder.generateDeserializer(type: ApType, adapterClass: IrClass): IrExpression {
  return when (type) {
    ...
    is ApFragmentType -> {
      val fragment = visitor.apolloCache
        .getFragment(type.name)
      if (fragment == null) {
        apIrError(visitor.pluginContext, "cannot find fragment '${type.name}'")
      } else {
        apIrGetObject(
          fragment
...
fun IrBuilder.generateSerializer(type: ApType, element: IrExpression, adapterClass: IrClass): IrExpression {
  return when (type) {
    ...
    is ApFragmentType -> {
      val fragment = visitor.apolloCache.getFragment(type.name)
      if (fragment == null) {
        apIrError(visitor.pluginContext, "cannot find fragment '${type.name}'")
      } else {
        apIrGetObject(
...
internal fun ApType.toIrType(adapterClassId: ClassId, apolloCache: ApolloCache): IrType {
  ...
  return when (this) {
    ...
    is ApFragmentType -> {
      ...
      if (fragment == null || fragmentDefinition == null) {
        visitor.pluginContext.irBuiltIns.nothingType
      } else {
        fragment.createNestedClassId(Identifiers.Data).toIrClassSymbol().defaultType.let {
