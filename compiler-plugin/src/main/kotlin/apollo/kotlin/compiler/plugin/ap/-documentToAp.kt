    ...
    val properties = mergedFields.entries.map { mergedFieldEntry ->
      ...
      when (val first = list.first()) {
        ...
        is GQLInlineFragment -> {
          ...
          val typeCondition = first.typeCondition?.name ?: contextType
          var subType: GQLType = GQLNamedType(name = typeCondition)
          if (typeCondition == contextType) {
            // TODO refine this, we can make this assumption in more cases
            subType = GQLNonNullType(sourceLocation = null, type = subType)
          }
          walkField(
            name = mergedFieldEntry.key,
            type = subType,
        ...
        is GQLFragmentSpread -> {
          ApProperty(
            name = mergedFieldEntry.key,
            type = ApFragmentType(name = first.name, parentType = contextType),
            model = null,
            deprecationReason = null,
            isFragment = true,
          )
